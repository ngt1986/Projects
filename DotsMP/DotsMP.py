...
def draw_lines(x, y, w, h, inactive_color, active_color, row, column):
    ...
    if x + w > mouse[0] > x and y + h > mouse[1] > y:
        ...
        if click[0] and h == dot_size * 2:
            ...
            # Send a move that was made to Server
            send_data_horiz(net, row, column)
...
def makes_box(player_turn, row, column, h):
    ...
    for j in range(1, grid_n + grid_n - 2):
        for i in range(1, grid_n + grid_n - 2):
            if game_board[i][j] == " ":
                if game_board[i + 1][j] == "-" and game_board[i - 1][j] == "-" and game_board[i][j - 1] == "|" and \
                        game_board[i][j + 1] == "|":
    ...
    for row in range(0, grid_n):
        for column in range(0, grid_n):
            pygame.draw.circle(DISPLAYSURF, green, (int((column + 1) * (display_width) / grid_n - display_margin_width),
                                                    int((row + 1) * (
                                                        display_height) / grid_n - display_margin_height / 2)),
                               dot_size)
...
def send_data_horiz(net, row, column):
        """
        Send horizontal line move to server
        :return: None
        """
        #ID : (row,column) : horiz (0) or vert(1)
        data = [str(net.id) + ":" + str(row) + "," + str(column) + ":" + "0"]
        reply = net.send(data)
        return reply


def send_data_vert(net, row, column):
    """
    Send vertical line move to server
    :return: None
    """
    # ID : (row,column) : horiz (0) or vert(1)
    data = [str(net.id) + ":" + str(row) + "," + str(column) + ":" + "1"]
    reply = net.send(data)
    return reply

def parse_data(data):
    try:
        d = data[0].split(":")[1].split(",")
        #d[0] = row and d[1] = column
...
def run():
    ...
    while not game_exit: #event handling
        ...
        #Update with other player's move
        game_board
